...
DEFAULT_SERVER_HOSTNAME_SUFFIX = config.get('defaults', {}).get('default_server_hostname_suffix', 'voidhost.pro')
...
class ConvoyBot(commands.Bot):
    ...
    async def close(self):
        logger.info("Shutting down Void Nodes bot...")
        await super().close()
...
async def create_user(interaction: discord.Interaction, discord_user: discord.User, email: str, is_admin: bool = False):
    ...
    if response_data and isinstance(response_data.get('data'), dict):
        ...
        success_msg_dm = (
            f"Your Void Nodes panel account has been created!\n\n"
        ...
        try:
             ...
             await dm_channel.send(embed=discord.Embed(
                  ...
                  title=f"{Elookup('success', '🎉')} Your Void Nodes Panel Account is Ready!",
...
async def create_vps(interaction: discord.Interaction):
    ...
    if admin_conf_view.status is True: # Approved by admin
        ...
        if creation_response and isinstance(creation_response.get('data'), dict):
            ...
            dm_embed = discord.Embed(title=f"{Elookup('success')} Your Void Nodes VPS is Ready!", description=details_desc, color=discord.Color.brand_green())
...
async def credits_cmd(interaction: discord.Interaction): 
    embed = discord.Embed(title=f"{Elookup('credits')} Void Nodes Bot - Credits", description="This bot was lovingly crafted for the Void Nodes community by:", color=discord.Color.gold())
    ...
    embed.add_field(name="✨ Void Nodes Project", value="[Visit our Website](https://voidhost.pro)", inline=False)
    if bot.user and bot.user.avatar:
        embed.set_thumbnail(url=bot.user.avatar.url)
    embed.set_footer(text="Thank you for using the Void Nodes Bot! Your Vision, Virtually Realized. ❤️")
...
async def get_fake_vps(interaction: discord.Interaction):
    ...
    # Second (or more) time, send the DM and a different ephemeral message
        ...
        if usage_counter[user_id] > 5: # Avoid spamming DMs if user keeps trying
...
class HelpView(discord.ui.View):
    ...
    def get_commands_for_category(self, category: str) -> List[Dict[str, str]]:
        commands_map = {
            ...
            "utility": [
                ...
                {"name": "/credits", "desc": "Shows credits for the bot creators and the Void Nodes project."},
                ...
                {"name": "/links", "desc": "Shows a list of useful links related to Void Nodes (Panel, Website, Discord)."},
    ...
    async def update_help_embed(self, interaction: discord.Interaction, category: str): # interaction is from select
        embed = discord.Embed(title=f"{Elookup('help', '🤖')} Void Nodes Bot Help - {category.title()} Commands",
    ...
    async def start(self, interaction: discord.Interaction): # interaction is the original /help command
        ...
        embed = discord.Embed(title=f"{Elookup('help', '🤖')} Void Nodes Bot - Command Help",
                              description="Welcome to the Void Nodes Bot! I'm here to help you manage your VPS and interact with the panel.\n\n"
...
async def help_cmd(interaction: discord.Interaction): 
    ...
    await view.start(interaction) # Start the interactive help view
...
class PlansView(discord.ui.View):
    ...
    def create_embed_for_category(self) -> discord.Embed:
        embed: discord.Embed
        title_prefix = f"{Elookup('plan_category','📜')} Void Nodes VPS Plans"
...
@bot.tree.command(name="links", description=f"{Elookup('links')} Shows a list of useful Void Nodes links.")
async def links_cmd(interaction: discord.Interaction): 
    embed = discord.Embed(
        title=f"{Elookup('links','🔗')} Void Nodes - Useful Links", 
        description="Here are some important links to help you navigate the Void Nodes ecosystem:",
        color=discord.Color.orange()
    )
    embed.add_field(name="🚀 Client & VPS Panel", value=f"Manage your services: <{PANEL_BASE_URL}>", inline=False)
    embed.add_field(name="✨ Void Nodes Official Website", value="Find out more about us: https://voidhost.pro", inline=False)
    ...
    if bot.user and bot.user.avatar:
        embed.set_thumbnail(url=bot.user.avatar.url)
    embed.set_footer(text="Void Nodes - Your Vision, Virtually Realized.")
    await interaction.response.send_message(embed=embed, ephemeral=False)
...
async def on_ready():
    ...
    activity_name = activity_conf.get("name", "over Void Nodes") # Default name
    activity_type_str = activity_conf.get("type", "watching").lower()
    stream_url = activity_conf.get("stream_url", "https://www.twitch.tv/voidnodes") # Example
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        ...
        exit(1)
    try:
        logger.info("Void Nodes Bot is preparing to launch...")
